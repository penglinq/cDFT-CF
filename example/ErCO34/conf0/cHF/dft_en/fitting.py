...
if __name__ == '__main__':
    ...
    symmetry = None
    j = 7.5
    nidx_lst = ['all']  #'all' # an integer or 'all'. Calculate Bkq with only a randomly chosen portion of data
    nrun = 1
    print_JM = False
    ...
    for nidx in nidx_lst:
        ...
        from stevens.cf_parameter import rot_proj
        proj_ori = rot_proj(proj_ori, np.loadtxt('../../rot_angle.txt').reshape(3), j)
        ...
        for e in np.average(ew_lst, axis=0):
            log.info("%s," % e)
        max_idx = np.argmax(la.norm(ew_lst - np.average(ew_lst, axis=0), axis=1))
        log.info("relative energy (deviate most) in cm^-1")
        for e in ew_lst[max_idx]:
            log.info("%s," % e)
        ...
        feri.close()
        np.save("ew_lst_%ssample.npy"%nidx, ew_lst)
        np.save("CF_para.npy", np.average(x_lst, axis=0)[:-1])
        H = crystal_field_H(np.average(x_lst, axis=0)[:-1], j, symmetry=symmetry)
        print(np.max(np.abs(H - H.T.conj())))
        ew, ev = la.eigh(H)
        print(ew - min(ew)) 
        # print eigenstate wavefunctions in |JM> basis
        for i in range(len(ew)):
        ...
        print_Bkq(np.average(x_lst, axis=0)[:-1], symmetry=symmetry, mode='phi')
        #print_Bkq(np.average(x_lst, axis=0)[:-1], symmetry=symmetry, mode='latex', hf_x=np.load('../x_HF_0.npy'))
        exit()
