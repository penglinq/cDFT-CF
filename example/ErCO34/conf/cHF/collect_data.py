...
# User input
c2_min = 0.90
J_gap = 500 # expected minimal gap between ground state J and 1st excited J manifold, in cm-1
...
'''
Jnum_dict = {}
with open('J_number.out') as file:
    for line in file:
        tmp = line.split()
        Jnum_dict[tmp[0]] = tmp[1]
Jnum_lst = []
converged_lst = []
for idx, name in enumerate(name_lst[sort_energy_idx[:32]]):
    if name in Jnum_dict:
        Jnum_lst.append(Jnum_dict[name])
        converged_lst.append(idx)
Jnum_lst = np.asarray(Jnum_lst, dtype=float)
print(Jnum_lst)
print("J number")
for i in range(len(Jnum_lst)):
    print(Jnum_lst[i])
'''
