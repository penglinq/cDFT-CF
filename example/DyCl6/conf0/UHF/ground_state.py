'''
Generate the intrinsic atomic orbitals (IAO), a set of localized atomic orbitals for [DyCl6]-3.
To do that, we first calculate the UHF ground state of [DyCl6]-3 in the target basis (sarc-dkh2). 
The density matrix initial guess for UHF is obtained by projecting the GHF ground state in the 
minimal basis into the target basis.
'''
import pyscf
...
if __name__ == "__main__":
    ...
    chkfname_guess = 'uhf_gs_%s.chk'%geometry # initial guess. 'minao' for default PySCF guess; None for constrained HF; 
    ...
    c_lo = get_iao(mol, chkfname, from_umf=True, ao_ovlp=None, minao=minao)  
    import h5py
