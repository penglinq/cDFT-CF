...
if __name__ == '__main__':
    ...
    j = 7.5 
    nidx_lst = [10, 30, 50, 70, 90]  #'all' # an integer or 'all'. Calculate Bkq with only a randomly chosen portion of data
    nrun = 1000
    ...
    for functional in functional_lst:
        ...
        for nidx in nidx_lst:
            ...
            #feri['H'] = crystal_field_H(np.average(x_lst, axis=0), j, symmetry=symmetry)
            #feri['H'] = np.average(H_lst, axis=0)
            feri['E_spinfree'] = E_final #np.average(E_lst, axis=0)
            feri.close()
